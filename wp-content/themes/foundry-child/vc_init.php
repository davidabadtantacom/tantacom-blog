...
if(!( function_exists('ebor_vc_add_att') )){
	function ebor_vc_add_attr(){
		...
		//Añadimos selector para excluir categorías del blog.
		$final_blog_exclude_cats = array('Ninguna' => '0');

		if( is_array($blog_cats) ){
			foreach( $blog_cats as $cat ){
				$final_blog_exclude_cats[$cat->name] = $cat->term_id;
			}
		}

		$attributes = array(
			'type' => 'checkbox',
			'heading' => "Seleccionar las categorías que quiere excluir del blog",
			'param_name' => 'filter_exclude',
			'value' => $final_blog_exclude_cats
		);
